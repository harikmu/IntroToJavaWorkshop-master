...
public class RobotTreasureHunt implements KeyEventDispatcher{
	...
	private void spaceBarWasPressed() {
		...
		int robotXLocation = large.getX();  
		int robotYLocation = large.getY();
		...
		//      --make a pop-up tell the robot where to go next
			JOptionPane.showInputDialog("go to pirate");
		}
		// 8. Give the user subsequent clues at different locations on the image
		// (pirate robot, swamp, parrots, etc.)
		if(robotXLocation == 580 && robotYLocation == 320) {
			JOptionPane.showInputDialog("go to boy");
		}
		if(robotXLocation == 200 && robotYLocation == 390) {
			JOptionPane.showInputDialog("go to fish");
		}
		if(robotXLocation == 470 && robotYLocation == 550) {
			JOptionPane.showInputDialog("go to skull");
		}
